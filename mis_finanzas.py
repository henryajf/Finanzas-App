...
i=str(item).lower()
if any(x in i for x in [“mercadocredito”,“mercado credito”,“tarjeta”,“visa”,“mastercard”,“amex”,“credito”,“banco”,“financiamiento”,“financiacion”,“cuota”]): return “Credito/Financiacion”
...
resumen.columns=[“Categoria”,“Total ARS”]
...
if total_ing_ars > 0:
    if balance_ars >= 0:
        st.markdown(f'<div class="alert alert-g">Tus ingresos cubren el <strong>{balance_pct}%</strong> de tus gastos. Superavit: <strong>{fmt_ars(balance_ars)}</strong></div>',unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="alert alert-r">Deficit mensual: <strong>{fmt_ars(abs(balance_ars))}</strong> - Los gastos superan los ingresos registrados</div>',unsafe_allow_html=True)

if not vencidos.empty:
    items_v=" - ".join(f"<strong>{r['Item']}</strong> ({fmt_ars(r['Monto (ARS)'])})" for _,r in vencidos.iterrows())
...
if not proximos.empty:
    items_p=" - ".join(f"<strong>{r['Item']}</strong> ({r['Dia Pago'].strftime('%-d %b')})" for _,r in proximos.iterrows())
    st.markdown(f'<div class="alert alert-o">Proximos 3 dias - {items_p}</div>',unsafe_allow_html=True)
...
st.markdown(f"""<div class="metrics-6">
  ...
  <div class="mcard mc-{"g" if balance_ars>=0 else "r"}"><div class="mlbl">Balance</div><div class="mval-sm" style="color:{balance_color}">{balance_sign}{fmt_k(balance_ars)}</div><div class="msub">{"Superavit" if balance_ars>=0 else "Deficit"}</div></div>
...
if total_ing_ars > 0:
    ...
    st.markdown(f"""<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px;">
      <div class="mcard" style="border-color:rgba(0,158,227,.2)"><div class="mlbl" style="color:{ACCENT}">Henry</div><div class="mval-sm" style="color:{ACCENT}">{fmt_k(ing_henry)}</div><div class="msub">{fmt_usd_val(ing_henry/dolar)} - {pct_h}%</div></div>
      <div class="mcard" style="border-color:rgba(168,85,247,.2)"><div class="mlbl" style="color:{PURPLE}">Jaike</div><div class="mval-sm" style="color:{PURPLE}">{fmt_k(ing_jaike)}</div><div class="msub">{fmt_usd_val(ing_jaike/dolar)} - {pct_j}%</div></div>
    </div>""",unsafe_allow_html=True)
...
if st.session_state.show_add_ingreso:
    ...
    with i2: ing_persona=st.selectbox("De quien?",["Henry","Jaike"],label_visibility="visible",key="ing_persona")
    ...
    st.markdown(f'<div style="font-size:11px;color:{MUTED2};margin:8px 0 12px">Tasa fija al momento del ingreso: <strong style="color:{TEXT}">${dolar:,.0f} ARS/USD</strong></div>',unsafe_allow_html=True)
...
if df.empty:
    st.markdown(f'<div class="card card-pad" style="text-align:center;padding:48px;color:{MUTED2}"><div style="font-size:36px;margin-bottom:10px">x</div><div style="font-weight:600">Sin datos.</div></div>',unsafe_allow_html=True)
else:
    sb1,sb2=st.columns([1,2])
    with sb1:
        busqueda=st.text_input("Buscar",placeholder="Buscar gasto...",label_visibility="collapsed",key="busqueda_input")
    with sb2:
        n_pend_total=int(df["Pagado"].eq(False).sum())
        n_venc_total=len(df[(df["Pagado"]==False)&df["Dia Pago"].notna()&(df["Dia Pago"]<date.today())])
        cats_disponibles=(["Todas",f"Pendientes ({n_pend_total})",f"Vencidos ({n_venc_total})"]
            +sorted(df["Cat"].unique().tolist()))
        filtro=st.selectbox("Categoria",cats_disponibles,label_visibility="collapsed",key="filtro_select")
    st.markdown("<div style='height:8px'></div>",unsafe_allow_html=True)

    df_vista=df.copy()
    if filtro.startswith("Pendientes") or filtro.startswith("Vencidos"): df_vista=df_vista[df_vista["Pagado"]==False]
        ...
        if df_vista.empty:
            ...
            for cat in cats_orden:
                ...
                for idx,row in df_cat.iterrows():
                    ...
                    st.markdown(f'<div class="item-row" style="opacity:{opacity}"><div class="item-ico" style="background:{color}{bg}">{svg_icon(cat,color,size=22)}</div><div class="item-body"><div class="{nc}">{row["Item"]}</div><div>{venc_html(row)}</div></div><div class="item-right"><div class="{mc}">{fmt_ars(monto)}</div><div class="item-usd">{fmt_usd(monto,dolar)}</div></div></div>',unsafe_allow_html=True)
        st.markdown("</div>",unsafe_allow_html=True)
        ...
        if not pend_items.empty:
            with st.expander(f"Marcar como pagado ({len(pend_items)} pendientes)"):
                for idx,row in pend_items.iterrows():
                    ...
                    with col_n: st.markdown(f'<div style="font-size:13px;font-weight:600;padding:6px 0">{row["Item"]} - {fmt_ars(row["Monto (ARS)"])}</div>',unsafe_allow_html=True)
                    with col_b:
    ...
    with col_der:
        ...
        st.markdown('<div class="card card-pad"><div class="ctitle">Distribucion de gastos</div>',unsafe_allow_html=True)
        ...
        if total_ing_ars > 0:
            fig_bal=go.Figure(go.Pie(labels=["Ingresos","Gastos"],values=[total_ing_ars,total_ars],hole=0.6,marker=dict(colors=[GREEN,RED],line=dict(color=SURFACE,width=3)),textinfo="none",hovertemplate="<b>%{label}</b><br>%{value:,.0f}<extra></extra>"))
        ...
        st.markdown(f'<div class="card card-pad"><div class="ctitle">Resumen</div><div class="res-row"><span class="res-k">Total items</span><span style="font-weight:700">{len(df)}</span></div><div class="res-row"><span class="res-k">Pagados</span><span style="font-weight:700;color:{GREEN}">{n_pag}</span></div><div class="res-row"><span class="res-k">Pendientes</span><span style="font-weight:700;color:{ORANGE}">{n_pend}</span></div><div class="res-row"><span class="res-k">Vencidos</span><span style="font-weight:700;color:{RED}">{len(vencidos)}</span></div><div class="res-row"><span class="res-k">Prox. 3 dias</span><span style="font-weight:700;color:{YELLOW}">{len(proximos)}</span></div><div class="res-row"><span class="res-k">Ingresos</span><span style="font-weight:700;color:{GREEN}">{len(df_ing) if not df_ing.empty else 0}</span></div><div class="res-row" style="flex-direction:column;align-items:flex-start;gap:2px;border-bottom:none"><span class="res-k">Mayor gasto</span><span style="font-weight:700;color:{TEXT}">{mayor}</span><span style="font-size:11px;color:{MUTED2}">{fmt_ars(mayor_m)}</span></div></div>',unsafe_allow_html=True)

        pend_cat=df[df["Pagado"]==False].groupby("Cat")["Monto (ARS)"].sum().sort_values(ascending=False)
        if not pend_cat.empty:
            max_p=pend_cat.max()
            st.markdown('<div class="card card-pad"><div class="ctitle">Pendiente por categoria</div>',unsafe_allow_html=True)
    ...
    st.markdown(f'<div style="font-size:10px;font-weight:700;color:{MUTED2};letter-spacing:.1em;text-transform:uppercase;margin-bottom:18px">Analisis del mes</div>',unsafe_allow_html=True)

    st.markdown('<div class="card card-pad"><div class="ctitle">Gasto por categoria</div>',unsafe_allow_html=True)
    ...
    st.markdown('<div class="card card-pad"><div class="ctitle">Pagado vs Pendiente por categoria</div>',unsafe_allow_html=True)
    ...
    st.markdown('<div class="card card-pad"><div class="ctitle">Top 5 gastos mas altos</div>',unsafe_allow_html=True)
    top5=df.nlargest(5,"Monto (ARS)")
    for _,row in top5.iterrows():
        color=cat_color(row["Cat"]); pct_top=int(row["Monto (ARS)"]/total_ars*100) if total_ars>0 else 0
        st.markdown(f'<div class="item-row"><div class="item-ico" style="background:{color}20">{svg_icon(row["Cat"],color,size=22)}</div><div class="item-body"><div class="item-name">{row["Item"]}</div><div style="font-size:11px;color:{MUTED2}">{row["Cat"]} - {pct_top}%</div></div><div class="item-right"><div class="item-monto">{fmt_ars(row["Monto (ARS)"])}</div><div class="item-usd">{fmt_usd(row["Monto (ARS)"],dolar)}</div></div></div>',unsafe_allow_html=True)
    st.markdown("</div>",unsafe_allow_html=True)
...
st.markdown(f"""<div class="metrics">
  ...
  <div class="mcard mc-{"g" if balance_ars>=0 else "r"}"><div class="mlbl">Balance</div><div class="mval" style="color:{GREEN if balance_ars>=0 else RED}">{"+" if balance_ars>=0 else ""}{fmt_k(balance_ars)}</div><div class="msub">vs gastos del mes</div></div>
</div>""",unsafe_allow_html=True)
...
if st.session_state.show_add_ingreso:
    ...
    with i2: ing_persona2=st.selectbox("De quien?",["Henry","Jaike"],label_visibility="visible",key="ing_persona2")
    ...
    with i6:
        ing_fecha2=st.date_input("Fecha",value=hoy,label_visibility="visible",key="ing_fecha2")
    st.markdown(f'<div style="font-size:11px;color:{MUTED2};margin:8px 0 12px">Tasa fija al momento del ingreso: <strong style="color:{TEXT}">${dolar:,.0f} ARS/USD</strong></div>',unsafe_allow_html=True)
...
if df_ing.empty:
    st.markdown(f'<div class="card card-pad" style="text-align:center;padding:48px;color:{MUTED2}"><div style="font-size:36px;margin-bottom:10px">x</div><div style="font-weight:600">Sin ingresos registrados.</div></div>',unsafe_allow_html=True)
else:
    ing_personas=df_ing.groupby("Persona")["Monto ARS"].sum().reset_index()
    fig_ing=go.Figure(go.Pie(labels=ing_personas["Persona"],values=ing_personas["Monto ARS"],hole=0.55,marker=dict(colors=[ACCENT,PURPLE],line=dict(color=SURFACE,width=3)),textinfo="none",hovertemplate="<b>%{label}</b><br>%{value:,.0f}<extra></extra>"))
    ...
    st.markdown('<div class="card card-pad"><div class="ctitle">Distribucion por persona</div>',unsafe_allow_html=True)
...
if df.empty:
st.markdown(f’<div class="card card-pad" style="text-align:center;padding:48px;color:{MUTED2}"><div style="font-size:36px;margin-bottom:10px">x</div><div style="font-weight:600">Sin datos en Google Sheets.</div></div>’,unsafe_allow_html=True)
else:
st.markdown(f’<div style="font-size:13px;color:{MUTED2};margin-bottom:14px;font-weight:500">Edita, agrega o marca pagos aca. Los cambios se reflejan en Inicio al guardar.</div>’,unsafe_allow_html=True)
