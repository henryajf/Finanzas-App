...
/* ── NAV — HACK PARA MANTENER LADO A LADO EN MÓVIL ── */
...
# ─────────────────────────────────────────────
# 10. NAVEGACIÓN — 2 botones full width siempre lado a lado
# ─────────────────────────────────────────────
...
if st.session_state.screen == "inicio":
    ...
    if df.empty:
        ...
        with col_izq:
            cats_orden = df.groupby("Cat.").apply(lambda g: g["Pagado"].eq(False).sum()).sort_values(ascending=False).index.tolist()
            st.markdown('<div class="card"><div style="padding:16px 18px 4px" class="ctitle">Detalle de gastos</div>', unsafe_allow_html=True)
            for cat in cats_orden:
                df_cat = df[df["Cat."] == cat]; t_cat = df_cat["Monto (ARS)"].sum(); color = cat_color(cat)
                n_pend = int(df_cat["Pagado"].eq(False).sum()); badge = f'<span class="sec-badge">{n_pend} pend.</span>' if n_pend > 0 else ""
                icon_s = svg_icon(cat, color, size=16)
                st.markdown(f'<div class="sec-hdr"><div class="sec-hdr-icon" style="background:{color}25">{icon_s}</div><span class="sec-hdr-name">{cat}{badge}</span><span class="sec-hdr-total" style="color:{color}">{fmt_ars(t_cat)}</span></div>', unsafe_allow_html=True)
                for _, row in df_cat.iterrows():
                    paid = row["Pagado"]; monto = row["Monto (ARS)"]; usd_str = fmt_usd(monto, dolar); badge_venc = venc_html(row); icon_lg = svg_icon(cat, color, size=22)
                    opacity = "0.45" if paid else "1"; name_cls = "item-name-paid" if paid else "item-name"; monto_cls = "item-monto-paid" if paid else "item-monto"
                    st.markdown(f'<div class="item-row" style="opacity:{opacity}"><div class="item-ico" style="background:{color}{"20" if not paid else "10"}">{icon_lg}</div><div class="item-body"><div class="{name_cls}">{row["Ítem"]}</div><div>{badge_venc}</div></div><div class="item-right"><div class="{monto_cls}">{fmt_ars(monto)}</div><div class="item-usd">{usd_str}</div></div></div>', unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        with col_der:
            ...
            st.markdown(f'<div style="display:flex;border-top:1px solid {BORDER2};padding-top:14px;margin-top:4px"><div style="flex:1;text-align:center"><div style="font-size:9px;font-weight:700;color:{MUTED};letter-spacing:.08em;text-transform:uppercase">Pagado</div><div style="font-size:16px;font-weight:800;color:{GREEN};margin-top:4px">{fmt_k(pagado_ars)}</div></div><div style="width:1px;background:{BORDER2}"></div><div style="flex:1;text-align:center"><div style="font-size:9px;font-weight:700;color:{MUTED};letter-spacing:.08em;text-transform:uppercase">Pendiente</div><div style="font-size:16px;font-weight:800;color:{RED};margin-top:4px">{fmt_k(pend_ars)}</div></div></div>', unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
            n_pag = int(df["Pagado"].sum()); n_pend = len(df) - n_pag; n_venc = len(vencidos); n_prox = len(proximos); mayor = df.loc[df["Monto (ARS)"].idxmax(), "Ítem"] if not df.empty else "—"; mayor_m = df["Monto (ARS)"].max() if not df.empty else 0
            st.markdown(f'<div class="card card-pad"><div class="ctitle">Resumen</div><div class="res-row"><span class="res-k">Total ítems</span><span style="font-weight:700">{len(df)}</span></div><div class="res-row"><span class="res-k">Pagados</span><span style="font-weight:700;color:{GREEN}">{n_pag}</span></div><div class="res-row"><span class="res-k">Pendientes</span><span style="font-weight:700;color:{ORANGE}">{n_pend}</span></div><div class="res-row"><span class="res-k">Vencidos</span><span style="font-weight:700;color:{RED}">{n_venc}</span></div><div class="res-row"><span class="res-k">Próx. 3 días</span><span style="font-weight:700;color:{YELLOW}">{n_prox}</span></div><div class="res-row" style="flex-direction:column;align-items:flex-start;gap:2px;border-bottom:none"><span class="res-k">Mayor gasto</span><span style="font-weight:700;color:{TEXT}">{mayor}</span><span style="font-size:11px;color:{MUTED2}">{fmt_ars(mayor_m)}</span></div></div>', unsafe_allow_html=True)
            pend_cat = df[df["Pagado"]==False].groupby("Cat.")["Monto (ARS)"].sum().sort_values(ascending=False)
            if not pend_cat.empty:
                max_p = pend_cat.max(); st.markdown('<div class="card card-pad"><div class="ctitle">Pendiente por categoría</div>', unsafe_allow_html=True)
    ...
    if df.empty:
        ...
        with bc1:
            if st.button("💾 Guardar y Sincronizar", type="primary", use_container_width=True):
                try:
                    df_up = df_edit.copy(); df_up["Categoría"] = df_up["Ítem"].apply(categorizar_inteligente); df_up = df_up[["Categoría","Ítem","Monto (ARS)","Día Pago","Pagado"]]; df_up["Día Pago"] = df_up["Día Pago"].apply(lambda x: str(x) if pd.notnull(x) else ""); df_up["Pagado"] = df_up["Pagado"].apply(lambda x: "TRUE" if x else "FALSE"); st.cache_data.clear()
                    hoja = get_gspread().open("Gastos_Henry").sheet1; hoja.clear(); hoja.append_row(df_up.columns.tolist()); hoja.append_rows(df_up.values.tolist()); st.success("✓ Cambios guardados en Google Sheets"); st.rerun()
